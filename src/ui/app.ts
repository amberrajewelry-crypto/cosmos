...
// Таймер растворения стартует с первого движения мыши — не раньше: гость должен успеть прочитать титры.
if (matchMedia('(hover: hover) and (pointer: fine)').matches) ['pointermove', 'pointerdown', 'keydown', 'wheel'].forEach((e) => window.addEventListener(e, wake, { passive: true }));
