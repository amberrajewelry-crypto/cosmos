...
export function openHonesty(overlay: HTMLElement): void {
  ...
  overlay.innerHTML = `
    <div class="honesty-box">
      ...
      <p class="honesty-p"><b>Контент-база (77 параметров по уровням масштаба):</b> у каждого указан источник и тег, 72 из них сверены (проходы A4 от 14–16.09: JPL Horizons, BGS, Planck 2018, HPS, LLR, первоисточники по клеткам и Галактике), 5 оставшихся считаются из твоей точки и живого Kp — их сверяют эфемериды, не константы. Тег [ТОЧНО] несверенному параметру структурно не выдаётся. Найденные расхождения исправлены и записаны: наклон магнитного диполя был 11° (учебное значение 1980-х) — теперь 9.3° по IGRF; поток частиц тёмной материи сквозь тело был завышен в ~2000 раз (10¹² → 5·10⁸ в секунду).</p>
      <p class="honesty-p"><a href="/changelog/" style="color:var(--gold)">Что изменилось — публичный журнал →</a> · <a href="https://github.com/amberrajewelry-crypto/cosmos/issues" style="color:var(--gold)" rel="noopener">Исходники и Issues на GitHub →</a> Нашёл ошибку — кнопка «число неверно» на карточке; после подтверждения тег снимается до исправления.</p>
    </div>`;
